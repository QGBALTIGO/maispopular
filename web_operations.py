...
def install_operations(app, panel, authenticated):
    ...
    async def forbidden(_: Request, __: PermissionError):
        return JSONResponse({"detail": "Acesso exclusivo de administrador."}, status_code=403)

    @app.exception_handler(PaymentError)
    async def unavailable(_: Request, __: PaymentError):
        return JSONResponse({"detail": "Não foi possível consultar o pagamento. Tente novamente."}, status_code=503)
    ...
    def payment_json(row, include_qr=True):
        ...
        data = {"id": row["id"], "status": status, "amount": row["amount_cents"] // 100,
        ...
        if include_qr:
            ...
            if data["qrCode"]:
                ...
                data["qrImage"] = "data:image/png;base64," + base64.b64encode(buffer.getvalue()).decode()
            balance = panel.store.balance_cents(row["user_id"])
            data.update(balance=f"{Decimal(balance)/100:.2f}", balanceLabel=money_brl(Decimal(balance)/100))
        return data

    @app.get("/api/payments")
    async def payments(init_data: Auth):
        uid = user_id(init_data)
        rows = panel.store.db.execute("SELECT * FROM payments WHERE user_id=? ORDER BY created_at DESC,rowid DESC LIMIT 20", (uid,))
    ...
    async def create_payment(payload: PaymentInput, init_data: Auth):
        ...
        customer = validate_customer(payload.name, payload.email, payload.phone, payload.document)
        row = await panel.create_payment(uid, payload.amount, customer, str(payload.request_id))
    ...
    async def order_detail(token: str, init_data: Auth):
        ...
        return {"id": token, "serviceName": row["service_name"], "state": row["state"],
                "status": row["provider_status"], "costLabel": money_brl(row["cost"]),
    ...
    async def refresh_order(token: str, init_data: Auth):
        ...
        if not row:
            raise HTTPException(404, "Pedido não encontrado.")
        if row["state"] == "SUBMITTED" and row["provider_id"] and row["checked_at"] < time.time() - 10:
            panel.store.update_status(token, await panel.api.status(row["provider_id"]))
        return {"ok": True}

    @app.post("/api/orders/{token}/actions/{kind}")
    async def prepare_action(token: str, kind: Literal["refill", "cancel"], init_data: Auth):
    ...
    async def admin_overview(init_data: Auth):
        ...
        unknown = panel.store.db.execute("SELECT id,user_id,service_name,cost FROM orders WHERE state='UNKNOWN' ORDER BY created_at LIMIT 30")
        credits = panel.store.db.execute("SELECT * FROM admin_credits ORDER BY created_at DESC,rowid DESC LIMIT 20")
        return {"stats": stats, "unknownOrders": [dict(r) for r in unknown], "credits": [dict(r) for r in credits]}

    @app.get("/api/admin/users")
    async def admin_users(init_data: Auth, search: str = Query(default="", max_length=80)):
        admin(init_data)
        term = search.strip().lstrip("@").replace("%", "").replace("_", "")
        rows = panel.store.db.execute("""SELECT u.user_id,u.username,u.display_name,w.balance_cents FROM users u
    ...
    async def admin_credit(payload: CreditInput, init_data: Auth):
        return panel.grant_credit(admin(init_data), payload.user_id, payload.amount, payload.reason, str(payload.request_id))
    ...
    async def resolve(token: str, payload: ResolveInput, init_data: Auth):
        ...
        async with panel.mutation_lock:
            ...
            if payload.decision == "naocriado":
                panel.store.reject_order_and_refund(token, f"Verificação administrativa por {admin_id}", "Pedido não criado; saldo devolvido.")
            else:
