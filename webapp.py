...
def validate_init_data(raw: str, bot_token: str, *, max_age: int = 3600,
...
def public_order(row: dict, balance_cents: int) -> dict:
    return {
        "id": row["id"], "state": row["state"], "serviceName": row["service_name"],
...
def create_app(panel: Panel, *, own_resources: bool = False) -> FastAPI:
    ...
    app = FastAPI(title="Mais Popular", docs_url=None, redoc_url=None,
    ...
    def authenticated(init_data: str) -> dict:
        ...
        panel.store.register_user(int(user["id"]), str(user.get("username", "")),
                                  " ".join(filter(None, (str(user.get("first_name", "")),
                                                                 str(user.get("last_name", ""))))))
        return user

    @app.exception_handler(ProviderError)
    async def provider_error(_: Request, __: ProviderError):
        return JSONResponse({"detail": "O catálogo está temporariamente indisponível."}, status_code=503)
    ...
    async def bootstrap(init_data: Annotated[str, Header(alias="X-Telegram-Init-Data", max_length=8192)]):
        user = authenticated(init_data)
        services = await panel.catalog()
        counts = {name: sum(s.platform == name for s in services)
    ...
    @app.get("/api/catalog")
    async def catalog(platform: str, init_data: Annotated[str, Header(alias="X-Telegram-Init-Data", max_length=8192)]):
    ...
    async def quote(payload: QuoteInput, init_data: Annotated[str, Header(alias="X-Telegram-Init-Data", max_length=8192)]):
        user = authenticated(init_data)
        try:
            row = await panel.quote(int(user["id"]), payload.service_id, payload.target,
                                    payload.value, payload.answer)
        except ValueError as exc:
...
def default_app() -> FastAPI:
    ...
    panel = Panel(settings, ServiceProvider(settings.api_key),
